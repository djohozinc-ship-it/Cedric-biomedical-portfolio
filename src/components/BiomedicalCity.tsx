...
const BiomedicalCity:React.FC=()=>{
 ...
 useEffect(()=>{
  ...
  try{
   ...
   box(g.entry,-2.1,3,0,3.7,6,1.2,dark);box(g.entry,2.1,3,0,3.7,6,1.2,dark);const dl=box(g.entry,-1.05,2.6,.15,1.8,4.8,.12,glass),dr=box(g.entry,1.05,2.6,.15,1.8,.12,glass);box(g.entry,0,5.2,.15,5.8,.1,.1,cyan);box(g.checkin,0,1.5,0,2.2,3,.65,dark);box(g.checkin,0,2.35,-.38,1.55,1.15,.08,cyan);
   ...
   const clock=new THREE.Clock(),target=new THREE.Vector3(),a=new THREE.Vector3(),b=new THREE.Vector3(),ta=new THREE.Vector3(),tb=new THREE.Vector3();const move=(p:number,from:Vec3Tuple,to:Vec3Tuple,l1:Vec3Tuple,l2:Vec3Tuple)=>{camera.position.lerpVectors(a.set(...from),b.set(...to),p);target.lerpVectors(ta.set(...l1),tb.set(...l2),p);camera.lookAt(target);};const show=(names:string[])=>Object.keys(g).forEach(k=>g[k].visible=names.includes(k));
