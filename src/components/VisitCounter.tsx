...
function readAndIncrementVisits() {
  try {
    const stored = Number.parseInt(window.localStorage.getItem(VISIT_STORAGE_KEY) ?? '0', 10);
    const visits = Number.isFinite(stored) && stored >= 0 ? stored + 1 : 1;
...
function VisitCounter() {
  ...
  return (
    <section className="visit-counter" aria-label={`${visits} visites de ce portfolio depuis ce navigateur`}>
      <div className="visit-counter-orbit" aria-hidden="true"><span /></div>
      <div className="visit-counter-copy">
        ...
        <span className="visit-counter-description">{visits === 1 ? 'première visite depuis ce navigateur' : 'visites depuis ce navigateur'}</span>
      </div>
      <div className="visit-counter-signal" aria-hidden="true"><i /><i /><i /></div>
    </section>
  );
}
