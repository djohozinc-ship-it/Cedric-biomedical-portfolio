        ...
        <main className="ppg-page">
            <section className="ppg-hero">
                ...
                <div className="ppg-hero-card">
                    ...
                    <div className="ppg-wave">
                        <span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span>
                    </div>
                    <div className="ppg-bpm">BPM<br /><strong>—</strong></div>
                    <div className="ppg-face-frame"><i></i></div>
                    <small>LIVE • VIDEO SIGNAL</small>
                </div>
            </section>

            <section className="ppg-section ppg-intro-grid">
                <div>
                    <span className="ppg-section-kicker">01 — CONTEXTE</span>
                    <h2>Transformer une vidéo en donnée physiologique exploitable</h2>
                </div>
                <div className="ppg-prose">
                    <p>Le projet explore le principe selon lequel de faibles variations photométriques observables dans une séquence vidéo peuvent contenir une composante périodique liée aux variations du volume sanguin.</p>
                    <p>L’objectif n’est pas de remplacer un dispositif médical validé, mais de construire et de tester une chaîne technique complète : acquisition, vision par ordinateur, extraction d’un signal temporel, traitement puis estimation du rythme cardiaque.</p>
                </div>
            </section>

            <section className="ppg-section">
                <div className="ppg-section-heading"><span className="ppg-section-kicker">02 — PROBLÉMATIQUE</span><h2>Comment récupérer un signal physiologique sans contact ?</h2></div>
                <div className="ppg-problem-card">
                    <FontAwesomeIcon icon={faBullseye} />
                    <div>
                        <strong>Question de départ</strong>
                        <p>Comment détecter une région du visage, convertir ses variations visuelles en signal temporel, puis exploiter cette signature pour obtenir une estimation du rythme cardiaque ?</p>
                    </div>
                </div>
            </section>

            <section className="ppg-section">
                <div className="ppg-section-heading"><span className="ppg-section-kicker">03 — CHAÎNE DE TRAITEMENT</span><h2>De la caméra au rythme cardiaque</h2></div>
                <div className="ppg-pipeline">
                    {pipeline.map(([n, title, text]) => <article key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></article>)}
                </div>
                <div className="ppg-architecture">Caméra <b>→</b> visage <b>→</b> ROI <b>→</b> signal vidéo <b>→</b> traitement <b>→</b> estimation BPM</div>
            </section>

            <section className="ppg-section ppg-tech-grid">
                <div className="ppg-tech-card"><FontAwesomeIcon icon={faLaptopCode} /><span>Vision par ordinateur</span><p>OpenCV pour la lecture, la manipulation et le traitement des images vidéo.</p></div>
                <div className="ppg-tech-card"><FontAwesomeIcon icon={faCogs} /><span>Suivi facial</span><p>MediaPipe pour stabiliser la localisation du visage et de la zone observée.</p></div>
                <div className="ppg-tech-card"><FontAwesomeIcon icon={faChartLine} /><span>Traitement du signal</span><p>Construction d’une série temporelle, analyse des variations et recherche d’une composante périodique exploitable.</p></div>
            </section>
            ...
            <section id="ppg-demo" className="ppg-section">
                ...
                <div className="ppg-video-box">
                    ...
                    <div className="ppg-video-overlay"><FontAwesomeIcon icon={faPlayCircle} /><span>Ajoute ta vidéo ici :<strong>/public/videos/projects/ppg/demo.mp4</strong></span></div>
                </div>
            </section>
            ...
            <section id="ppg-results" className="ppg-section">
                <div className="ppg-section-heading"><span className="ppg-section-kicker">06 — RÉSULTATS</span><h2>Résultats obtenus et éléments à documenter</h2></div>
                <div className="ppg-result-grid">{resultCards.map((item) => <article key={item.label}><small>{item.label}</small><strong>{item.value}</strong><p>{item.note}</p></article>)}</div>
                <div className="ppg-results-note"><FontAwesomeIcon icon={faChartLine} /><div><strong>Important</strong><p>Les mesures finales (BPM de référence, BPM estimé, écart, nombre d’essais, durée de capture, etc.) sont volontairement laissées éditables pour que tu puisses renseigner exactement les valeurs issues de tes expériences.</p></div></div>
            </section>

            <section className="ppg-section">
                <div className="ppg-section-heading"><span className="ppg-section-kicker">07 — ANALYSE</span><h2>Ce que le projet démontre</h2></div>
                <div className="ppg-analysis-grid">
                    <article><h3>Compétence technique</h3><p>Relier vision par ordinateur, extraction de données et traitement du signal dans une même chaîne expérimentale.</p></article>
                    <article><h3>Approche expérimentale</h3><p>Observer le comportement réel du signal, identifier les sources de variation et itérer sur la méthode d’extraction.</p></article>
                    <article><h3>Limites</h3><p>La lumière, les mouvements, la qualité vidéo, la peau visible et la stabilité de la région analysée influencent fortement le signal extrait.</p></article>
                    <article><h3>Perspective</h3><p>Améliorer la robustesse de la chaîne, documenter les mesures comparatives et étudier la reproductibilité sur plusieurs acquisitions.</p></article>
                </div>
            </section>
